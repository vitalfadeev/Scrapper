...
from .Scrapper_Merger_Item import WordItem

# constants
DB_NAME                 = "words.db"
DBWords                 = sqlite3.connect(DB_NAME, isolation_level         = None)
...
# init DB
DBExecute( DBWords, "PRAGMA journal_mode = OFF" )
DBExecuteScript( DBWords, WordItem.Meta.DB_INIT )
...
def SearchWikictionary( item: WikidataItem ):
    yield from DBRead( DBWikictionary, item.LabelName, where="LabelName = ?" )
...
def wikidata_reader():
    yield from DBRead( DBWWikidata, WikidataItem.Meta.DB_TABLE_NAME )
...
def mainfunc_single_process():
    ...
    for WikidataItem in wikidata_reader():
        ...
        for id in ids:
            for wikictionary_item in DBRead( DBWikictionary, WikictionaryItem.Meta.DB_TABLE_NAME, where="PrimaryKey=", args=(id,)  ):
                word = MergeWiktionary( word, wikictionary_item )
                words.append( word )
