...
def is_ascii(s):
    """ Return True if all symbols is ASCII (code < 128) """
...
class Container:
    ...
    def wrap(self, element):
        self.parent.add_child(element, before=self)
    ...
    def dumps(self, level=0, types=None):
        s = "  "*level + repr(self) + "\n"

        for child in self.childs:
            if types:
                if isinstance(child, types):
                    s += child.dumps(level+1, types)
            else:
                s += child.dumps(level+1, types)
    ...
    def dump(self, level=0, types=None, prefix=""):
        print("  "*level + prefix + ' ' + repr(self))
        i = 1
        ...
        for child in self.childs:
            ...
            if types:
                if isinstance(child, types):
                    child.dump(level+1, types, prefix2)
                    i += 1
            else:
                child.dump(level+1, types, prefix2)
                i += 1
...
class Template(Container):
    ...
    def to_words( self ) -> typing.List[str]:
        yield from ()
...
assert repr(tagizer("<a>123</a>")) == "(10, [Html(a, [])])"
...
def pack_lists(lexems: list) -> list:
    ...
    for c in lexems:
        if isinstance(c, Li):
            ...
            elif isinstance(last, Li):
                if is_child_li(last, c):
                    last.add_child(c)
                    last = c
                    
                else:
                    while not is_child_li(last, c):
                        last = last.parent
                        if not isinstance(last, Li):
                            break
                        
                    if isinstance(last, Li):
                        last.add_child(c)
