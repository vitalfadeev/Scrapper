...
def scrap_load_page_test(lang="en", label="cat"):
    id_   = 0
    ns    = 0
    text  = get_test_file_content(lang, label)
    page  = Scrapper_Wiktionary.Page(id_, ns, label, text)
    items = Scrapper_Wiktionary_EN.load_external_page( label )
    return  items


if __name__ == "__main__":
    lang = 'en'
    title = 'Thesaurus:book'

    setUp()
    items = Scrapper_Wiktionary_EN.load_external_page( title )
    print( items )
